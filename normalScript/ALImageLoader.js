var ALImageLoader = (function(){
    ...
    var self = {
            ...
            if(jsb){
                ...
                function loadEnd(){
                    cc.loader.load(filepath,function(err,texture){
                        if(err){
                            self._doTaskFinish(err,url,null);
                        }else{
                ...
                function saveFile(data){
                    if(data){
                        ...
                        if(jsb.fileUtils.writeDataToFile(new Uint8Array(data),filepath)){
                            ...
                            let error = {status:-100,errorMessage:"Remote write file failed"};
                            self._doTaskFinish(error,url,null);
                        }
                        
                    }else{
                        cc.log("Download file failed");
                        let error = {status:-1,errorMessage:"Download file failed"}
                        self._doTaskFinish(error,url,null);
                    }
                }
        ...
        _doTaskFinish:function(err,url,sf){
            ...
            // 移除任务列表中的任务
            try {
                // let index = self._tasks.indexOf(url);
                // if(index !== -1){
                //     // 移除任务
